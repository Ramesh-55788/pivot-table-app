...
const PivotTable = ({ pivotData, rowFields, colFields }) => {
  const { pivotMap, colKeys } = pivotData;
  ...
  const buildHeader = () => {
    ...
    for (let level = 0; level < levels; level++) {
      ...
      if (level === 0) {
        row.push(
          <th key="corner" rowSpan={levels + 1} colSpan={hasRowFields ? rowFields.length : 1} style={{ backgroundColor: '#eee' }}>
            {hasRowFields ? rowFields.join(" / ") : 'Values'}
          </th>
        );
      }
      ...
      for (let i = 0; i <= splitHeaders.length; i++) {
        ...
        if (current === prev) {
          ...
          if (prev !== null) {
            row.push(
              <th key={`header-${level}-${i}`} colSpan={span} style={{ backgroundColor: '#eee' }}>
                {prev || '-'}
              </th>
            );
          }
  ...
  const renderBody = () => {
      ...
      if (rowFields.length > 0) {
        for (let level = 0; level < rowFields.length; level++) {
          ...
          if (rowSpanMap.has(key)) {
            tds.push(
              <td key={`rowkey-${key}`} rowSpan={rowSpanMap.get(key)} style={{ backgroundColor: '#f9f9f9' }}>
                {splitKey[level] || '-'}
              </td>
            );
          }
    ...
    const totalRow = [
      <td key="total-label" colSpan={rowFields.length || 1} style={{ fontWeight: 'bold', backgroundColor: '#f0f0f0' }}>
        Total
      </td>,
      ...colTotals.map((v, i) => (
        <td key={`col-total-${i}`} align="right" style={{ fontWeight: 'bold', backgroundColor: '#f0f0f0' }}>
          {v.toFixed(2)}
        </td>
      )),
      <td key="grand-total" align="right" style={{ fontWeight: 'bold', backgroundColor: '#f0f0f0' }}>
